...
# §fix (codec detection bug): the importable module name for each of these
# packages is NOT the same as the pip package name -- e.g. `pip install
# pylibjpeg-libjpeg` installs a module you import as `libjpeg`, not as
# `pylibjpeg_libjpeg`. The old check used the pip names for __import__(),
# which meant it always reported these as missing even when correctly
# installed. Map import-name -> pip-package-name so detection is correct
# AND the install hint in the log message names real installable packages.
_CODEC_PLUGINS = (
    ("pylibjpeg", "pylibjpeg"),
...
for _import_name, _pip_name in _CODEC_PLUGINS:
    try:
        __import__(_import_name)
    except Exception:
        MISSING_CODEC_PLUGINS.append(_pip_name)
...
DEFAULT_APP_SETTINGS = {
    ...
    "show_dashboard_network_ip": True,  # local IP lookup on Dashboard is a background DNS call every ~30s;
                                        # turn off on machines with slow/unreachable DNS (domain-joined, VPN, etc.)
    # ---- Appearance ----
...
def build_admin_only_tabs():
    ...
    net_ip_ctrl = _settings_row(perf_body, "Show Network IP on Dashboard",
                                "Looks up this machine's local IP address to display on the Dashboard. "
                                "On some networks (domain-joined PCs, VPNs, or slow/misconfigured DNS) "
                                "this lookup can be slow. Turning it off skips the lookup entirely.")
    settings_show_network_ip_var = ctk.BooleanVar(value=APP_SETTINGS.get("show_dashboard_network_ip", True))

    def do_settings_show_network_ip_change():
        _update_app_setting("show_dashboard_network_ip", settings_show_network_ip_var.get())
        try:
            refresh_home_dashboard()
        except Exception:
            pass

    ctk.CTkSwitch(net_ip_ctrl, text="", variable=settings_show_network_ip_var,
                  command=do_settings_show_network_ip_change).pack()
...
_local_ip_cache = {"value": "—", "inflight": False, "last_checked": 0.0}


def _kick_off_background_local_ip_check():
    """Non-blocking local IP lookup, throttled to at most once every 30s.
    socket.gethostbyname(socket.gethostname()) is a real DNS/network call --
    on domain-joined or VPN-connected client PCs with slow/unreachable DNS
    it can block for seconds. Doing it synchronously on every periodic
    refresh tick was freezing the whole UI on those machines. Result lands
    in _local_ip_cache for the NEXT dashboard refresh to display -- the
    current tick never blocks waiting on it."""
    if not APP_SETTINGS.get("show_dashboard_network_ip", True):
        return
    now = time.time()
    if _local_ip_cache["inflight"] or (now - _local_ip_cache["last_checked"]) < 30:
        return
    _local_ip_cache["inflight"] = True
    _local_ip_cache["last_checked"] = now

    def run():
        try:
            _local_ip_cache["value"] = socket.gethostbyname(socket.gethostname())
        except Exception:
            _local_ip_cache["value"] = "—"
        finally:
            _local_ip_cache["inflight"] = False
...
def refresh_home_dashboard():
    ...
    if APP_SETTINGS.get("show_dashboard_network_ip", True):
        _kick_off_background_local_ip_check()
        dash_home_recv_ip.configure(text=_local_ip_cache["value"])
    else:
        dash_home_recv_ip.configure(text="Disabled")
