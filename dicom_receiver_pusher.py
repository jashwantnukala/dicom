...
NSFALL_TRAER_SYNTAXES = list(dict.fromkeys(UID(str(ts)) for ts in _pydicom_uid.AllTransferSyntaxes))
...
def upsert_patient(pid, pname, institution, study_uid, modality, source, status=None, is_document=False):
...
WL_COLUMNS = (
    ...
    "count", "doc_count", "documents", "report", "history", "source", "status", "time", "sent_time", "push_target", "last_error",
)
...
WL_WIDTHS = {
    ...
    "modality": 95, "count": 70, "doc_count": 60, "documents": 175, "report": 175, "history": 175, "source": 190, "status": 170,
    ...
    def do_resend_selected_document():
        ...
        def run():
            try:
                ...
                app.after(0, lambda: dt_sent_status_lbl.configure(text=f"Resend failed: {e}", text_color=THEME_DANGER))
