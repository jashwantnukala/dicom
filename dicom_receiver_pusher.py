...
def save_daily_stats_history(history):
    try:
        with open(DAILY_STATS_HISTORY_FILE, "w", encoding="utf-8") as f:
...
def get_worklist_row_height():
    return _LARGE_WORKLIST_ROW_HEIGHT if APP_SETTINGS.get("larger_click_targets") else _BASE_WORKLIST_ROW_HEIGHT
...
def apply_ui_scaling():
    ...
    try:
        ctk.set_widget_scaling(max(0.5, APP_SETTINGS.get("ui_scaling_pct", 100) / 100.0))
    except Exception:
        pass
...
def save_export_template(cfg):
    try:
        with open(EXPORT_TEMPLATE_CONFIG_FILE, "w", encoding="utf-8") as f:
            json.dump(cfg, f, indent=2)
    except Exception:
...
def save_report_email_schedule(cfg):
    with open(REPORT_EMAIL_SCHEDULE_CONFIG_FILE, "w", encoding="utf-8") as f:
        json.dump(cfg, f, indent=2)
    _report_email_schedule_cache["value"] = cfg
...
def save_worklist_column_widths(data):
    try:
        with open(WORKLIST_COLUMN_WIDTHS_FILE, "w", encoding="utf-8") as f:
...
def populate_tree(tree, search="", force=False):
    ...
    paginated = id(tree) in _PAGINATED_TREES
    if paginated:
        st = _pagination_state(tree)
...
if not PSUTIL_AVAILABLE:
    ctk.CTkLabel(
        ...
        font=get_font("small"), text_color="#e67e22",
...
rec_select_all_btn = ctk.CTkButton(
    rec_search_frame, text="Select All", width=110,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: rec_tree.selection_set(rec_tree.get_children("")))
rec_select_all_btn.pack(side="left", padx=4)
rec_select_all_matching_btn = ctk.CTkButton(
    rec_search_frame, text="Select All Matching Filter", width=190,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _select_all_matching_filter(rec_tree))
rec_select_all_matching_btn.pack(side="left", padx=4)
rec_invert_selection_btn = ctk.CTkButton(
    rec_search_frame, text="Invert Selection", width=140,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _invert_tree_selection(rec_tree))
rec_invert_selection_btn.pack(side="left", padx=4)
rec_clear_selection_btn = ctk.CTkButton(
    rec_search_frame, text="Clear Selection", width=130,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _clear_persistent_selection(rec_tree))
rec_clear_selection_btn.pack(side="left", padx=4)
...
rec_wl_frame.pack(fill="x", padx=10, pady=(6, 4))
...
push_select_all_btn = ctk.CTkButton(
    push_search_frame, text="Select All", width=110,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: push_tree.selection_set(push_tree.get_children("")))
push_select_all_btn.pack(side="left", padx=4)
push_select_all_matching_btn = ctk.CTkButton(
    push_search_frame, text="Select All Matching Filter", width=190,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _select_all_matching_filter(push_tree))
push_select_all_matching_btn.pack(side="left", padx=4)
push_invert_selection_btn = ctk.CTkButton(
    push_search_frame, text="Invert Selection", width=140,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _invert_tree_selection(push_tree))
push_invert_selection_btn.pack(side="left", padx=4)
push_clear_selection_btn = ctk.CTkButton(
    push_search_frame, text="Clear Selection", width=130,
    fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER,
    command=lambda: _clear_persistent_selection(push_tree))
push_clear_selection_btn.pack(side="left", padx=4)
...
ctk.CTkLabel(push_search_frame, text="(Right-click a row for more actions)",
             font=get_font("small"), text_color=THEME_TEXT_MUTED).pack(side="left", padx=10)
...
push_wl_frame.pack(fill="x", padx=10, pady=(6, 4))
...
def do_settings_manual_backup():
    ...
    def run():
        try:
            ...
            def on_fail():
                settings_status_lbl.configure(text=f"Backup failed: {e}", text_color=THEME_DANGER)
            app.after(0, on_fail)
...
def do_settings_verify_integrity():
    ...
    text = " ".join(lines)
    color = THEME_SUCCESS if not missing and not corrupt else ("#e67e22" if not corrupt else THEME_DANGER)
...
def build_admin_only_tabs():
    ...
    routing_test_btn = ctk.CTkButton(routing_form_frame, text="Test Rule", width=110,
                                      fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER)
    routing_test_btn.pack(side="left", padx=(12, 2))
    ...
    log_export_filtered_btn = ctk.CTkButton(logs_top, text="Export Filtered to CSV", width=180,
                                             fg_color=THEME_NEUTRAL_BTN, hover_color=THEME_NEUTRAL_BTN_HOVER)
    log_export_filtered_btn.pack(side="left", padx=4)
    ...
    def _settings_row(parent, label_text, hint=None):
        row = ctk.CTkFrame(parent, fg_color="transparent")
        ...
        if hint:
            make_wrapped_label(left, hint, 520, font=get_font("micro"),
                                text_color=THEME_TEXT_MUTED).pack(anchor="w")
...
def do_import_config_bundle():
    ...
    if not modern_confirm(
        ...
        f"Backup Schedule, and Log Retention settings with the values from this bundle "
...
def save_destination_health_history(data):
    try:
        with open(DESTINATION_HEALTH_HISTORY_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)
    except Exception:
...
def refresh_home_dashboard():
    ...
    for key, canvas in dash_graph_canvases.items():
        if daily_trend is not None and key in ("studies_received", "studies_sent", "failed_transfers"):
...
def _confirm_large_push_if_needed(pids, dest_count=1):
...
def do_push_selected():
    ...
    if push_multi_dest_toggle_var.get():
        ...
        if not dests:
            ...
            return
        if not _confirm_large_push_if_needed(pids, dest_count=len(dests)):
            return
        threading.Thread(target=run_push_job_multi, args=(pids, dests),
                          kwargs={"anonymize": anon}, daemon=True).start()
    else:
        if not _confirm_large_push_if_needed(pids):
            return
...
def _stage_delete_to_trash(pid, patient_record):
    ...
    folder = os.path.join(OUTPUT_DIR, pid)
    trash_id = f"{pid}_{int(time.time() * 1000)}"
...
def _restore_from_trash(trash_id):
    ...
    if trash_path and os.path.isdir(trash_path):
        dest = os.path.join(OUTPUT_DIR, pid)
        try:
